...
def get_displacement(fixed, moving, alpha=(2,2), update_lag=10, iterations=20, min_level=0, levels=50, eta=0.8,
    ...
    for i in range(max(max_level_x, max_level_y), min_level - 1, -1):
        level_size = (int(round(m * eta**(min(i, max_level_y)))),
        ...
        # Compute hx, hy as intended and pass them to solver
        ...
        if i == max(max_level_x, max_level_y):
            ...
            tmp = imregister_wrapper(f2_level, u[1:-1,1:-1]/current_hx, v[1:-1,1:-1]/current_hy, f1_level)
        ...
        du, dv = level_solver(
            np.ascontiguousarray(J11), np.ascontiguousarray(J22), np.ascontiguousarray(J33),
            np.ascontiguousarray(J12), np.ascontiguousarray(J13), np.ascontiguousarray(J23),
            np.ascontiguousarray(weight_level),
            u, v, alpha, iterations, update_lag, 0, a_data_arr, a_smooth, current_hx, current_hy
        )
