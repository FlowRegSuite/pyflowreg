...
if __name__ == "__main__":
    ...
    print(f"Number of frames: {file_reader.frame_count}")

    idx = range(8000, 10000, 10)

    for i in idx:
        frame = file_reader.read_frames([i])
        cv2.imshow("Frame", cv2.normalize(frame[:, :, 0, 0], None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U))
        cv2.waitKey(1)