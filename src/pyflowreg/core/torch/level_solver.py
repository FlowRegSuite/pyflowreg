...
def level_solver_rbgs_torch(J11, J22, J33, J12, J13, J23, weight_level, u, v, alpha, iterations, update_lag, a_data,
        a_smooth, hx, hy, omega=1.95, eps=1e-6, update_lag_semantics: str = "torch", ):
    with torch.no_grad():
        m, n, K = J11.shape
        ...
        if isinstance(a_data, (float, int)):
            A_vec = torch.full((1, 1, K), float(a_data), dtype=J11.dtype, device=J11.device)
        else:
            A_vec = torch.as_tensor(a_data, dtype=J11.dtype, device=J11.device).view(1, 1, -1)
        W = weight_level if (weight_level.dim() == 3 and weight_level.shape == J11.shape) else weight_level.view(1, 1,
                                                                                                                 -1)
        ...
        for it in range(iterations):
            upd_tick = (it % update_lag) == 0 if update_lag_semantics == "torch" else ((it + 1) % update_lag) == 0
            if upd_tick:
                du3 = du.unsqueeze(-1)
                dv3 = dv.unsqueeze(-1)
                E = J11 * (du3 * du3) + J22 * (dv3 * dv3) + 2 * J12 * (du3 * dv3) + 2 * J13 * du3 + 2 * J23 * dv3 + J33
                E.clamp_min_(0)
            ...
            if a_smooth != 1:
                ...
                wL_v = wR_v = ax
                wT_v = wB_v = ay
            num_u[c1, c2] = (-torch.sum(W * psi_data * (J13 + J12 * dv.unsqueeze(-1)), dim=2)[c1, c2] + wL_u * (
                        u[c1, 0:n - 2] + du[c1, 0:n - 2] - u[c1, c2]) + wR_u * (
                                         u[c1, 2:n] + du[c1, 2:n] - u[c1, c2]) + wT_u * (
                                         u[0:m - 2, c2] + du[0:m - 2, c2] - u[c1, c2]) + wB_u * (
                                         u[2:m, c2] + du[2:m, c2] - u[c1, c2]))
            den_u[c1, c2] = denom_u_data[c1, c2] + (wL_u + wR_u + wT_u + wB_u)
            num_v[c1, c2] = (-torch.sum(W * psi_data * (J23 + J12 * du.unsqueeze(-1)), dim=2)[c1, c2] + wL_v * (
                        v[c1, 0:n - 2] + dv[c1, 0:n - 2] - v[c1, c2]) + wR_v * (
                                         v[c1, 2:n] + dv[c1, 2:n] - v[c1, c2]) + wT_v * (
                                         v[0:m - 2, c2] + dv[0:m - 2, c2] - v[c1, c2]) + wB_v * (
                                         v[2:m, c2] + dv[2:m, c2] - v[c1, c2]))
            den_v[c1, c2] = denom_v_data[c1, c2] + (wL_v + wR_v + wT_v + wB_v)
            upd_u = du[c1, c2]
            upd_v = dv[c1, c2]
            new_u = (1 - omega) * upd_u + omega * (num_u[c1, c2] / torch.maximum(den_u[c1, c2], epsv))
            new_v = (1 - omega) * upd_v + omega * (num_v[c1, c2] / torch.maximum(den_v[c1, c2], epsv))
            tmp = upd_u.clone()
