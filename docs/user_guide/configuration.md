...
Set `gnc_schedule=None` to keep the legacy solver path. When provided, PyFlowReg reruns
the pyramid once per stage, warm-starting each stage from the previous result.
